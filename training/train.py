...
def train():
    ...
    # Initialize
    init_seeds()  # init random seed
    if opt.multi_scale:  # multiscale training [sz/1.5, sz*1.5]
    ...
    train_path = 'C:\\Users\\44724\\data\\data_training'
    test_path = 'C:\\Users\\44724\\data\\sample1000'
    ...
    print('\n' + '-' * 10 + '\n' + 'model_params:')
    flag_conv = True
    pg0_name, pg1_name, pg2_name = [], [], []
    for k, v in dict(model.named_parameters()).items():
        print(k)
        if '.bias' in k:  # bias in pg2
            pg2 += [v]  # biases
            pg2_name.append(k)
        elif '.weight' in k and flag_conv:  # no params in pg1, maybe we should use .weight
    ...
    print(f'pg0:{pg0_name}')
    print(f'pg1(with weight decay):{pg1_name}')
    print(f'pg2(double lr):{pg2_name}\n')
    ...
    optimizer.add_param_group({'params': pg1, 'weight_decay': hyp['weight_decay']})  # add pg1 with weight_decay 4e-4
    optimizer.add_param_group({'params': pg2})  # add pg2 (biases)
    optimizer.param_groups[2]['lr'] *= 2.0  # double bias lr
    del pg0, pg1, pg2, pg0_name, pg1_name, pg2_name, flag_conv
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        # load model
        try:
            print("successfully resume model params.")
            pre_chkpt = chkpt['model'] 
            chkpt['model'] = {k: v for k, v in chkpt['model'].items() if model.state_dict()[k].numel() == v.numel()}
            if not pre_chkpt == chkpt['model']:
                print('trained model parameters has changed while loading!')
        ...
        if chkpt['optimizer'] is not None:
            print("successfully resume optimizer params.")
            optimizer.load_state_dict(chkpt['optimizer'])
            best_fitness = chkpt['best_fitness']

        # load results
        if chkpt.get('training_results') is not None:
            print("successfully resume previous training results.")
            with open(results_file, 'w') as file:
                file.write(chkpt['training_results'])  # write results.txt

        start_epoch = chkpt['epoch'] + 1
        print("successfully resume previous training epochs.\n")
        del chkpt
    ...
    lf = lambda x: (1 + math.cos(x * math.pi / epochs)) / 2 * 0.99 + 0.01  # cosine temp decay [1 : 0.01] https://arxiv.org/pdf/1812.01187.pdf
    scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lf)
    # scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[round(epochs * x) for x in [0.8, 0.9]], gamma=0.1)
    scheduler.last_epoch = start_epoch  # resume training epoch, increase by 1 every time scheduler.step()
    ...
    nw = min([os.cpu_count(), batch_size if batch_size > 1 else 0, 16])  # number of workers, dtermined by the cpu #threads
    ...
    #                     iou_thres=0.6,
    #                     save_json=False and is_coco,
    #                     single_cls=opt.single_cls,
    #                     dataloader=testloader)
    # print(f'\ninitial test iou:{results[0]}\n')

    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        model.train()
        model.gr = 1 - (1 + math.cos(min(epoch * 2, epochs) * math.pi / epochs)) / 2  # GIoU <-> 1.0 loss ratio, [0, 1]
        ...
        print(('\n' + '%10s' * 8) % ('Epoch', 'gpu_mem', 'GIoU', 'obj', 'cls', 'total', 'targets', 'img_size'))  # title for pb
        pbar = tqdm(enumerate(dataloader), total=nb)  # progress bar
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ni = i + nb * epoch  # number integrated batches (since train start) #iters
            ...
            if opt.multi_scale:
                if ni / accumulate % 1 == 0:  #  adjust img_size (67% - 150%) every accumulated batch
                    img_size = random.randrange(img_sz_min, img_sz_max + 1) * 32
                sf = img_size / max(imgs.shape[2:])  # scale factor
                if sf != 1:  # reshape the image by sf
                    ns = [math.ceil(x * sf / 32.) * 32 for x in imgs.shape[2:]]  # new shape (stretched to 16-multiple)
                    imgs = F.interpolate(imgs, size=ns, mode='bilinear', align_corners=False)

            # Run model
            pred = model(imgs)  # training: [bs, #anchors, h, w, #predictions]

            # Compute loss
            loss, loss_items = compute_loss(pred, targets, model)  # location, confidece, class, total loss
        ...
        if test_iou > test_best_iou:
            ...
            test_best_iou_info = f'best test iou: {test_best_iou}'
            print(test_best_iou_info)
            with open(results_file, 'a') as f:
                f.write(test_best_iou_info + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='C:\\Users\\44724\\data\\data_training', help='*.data path')
